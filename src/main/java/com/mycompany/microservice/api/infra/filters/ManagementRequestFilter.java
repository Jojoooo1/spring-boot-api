...
@WebFilter(
    asyncSupported = true,
    urlPatterns = {AppUrls.MANAGEMENT + "/*"})
@RequiredArgsConstructor
public class ManagementRequestFilter extends HttpFilter {

  @Serial private static final long serialVersionUID = 587106095561283733L;
  private static final String LOG_NAME = "ManagementRequestFilter";
  ...
  public void doFilter(
    ...
    log.debug("[{}] Verifying company exists and is_management", LOG_NAME);
    ...
    try {
      ...
      if (companyOptional.isPresent()
          && Boolean.TRUE.equals(companyOptional.get().getIsManagement())) {
        log.debug("[{}] company '{}' is_management authorized", LOG_NAME, companySlug);
        filterChain.doFilter(req, res);
      } else {
        log.info("[{}] company '{}' is_management is false, returning 401", LOG_NAME, companySlug);
        res.reset();
