...
import com.mycompany.microservice.api.utils.UrlUtils;
  ...
  public record PaginationLink<T>(
    ...
    private static String sortString(final Sort sort) {
      ...
      for (final Sort.Order s : sort) {
        ...
        ans.append(UrlUtils.encodeURLComponent(s.getProperty()));
        ans.append(",");
        ans.append(UrlUtils.encodeURLComponent(s.getDirection().name().toLowerCase()));
      }
